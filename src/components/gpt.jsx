...
(카테고리가 하나일때는 'Bar-chart', 'Line-chart중에 추천해주고 카테고리가 두 개 선택되었을 때는 'Bar-line','Scatter','Pyramid' 중에 골라서 데이터와 가장 잘 맞는 그래프 형태 추천)
- 사용자가 데이터를 제공하면, 데이터 간의 상관관계 등 주요 인사이트를 뽑아 2~3줄로 요약.(이때 시작은 그래프를 분석한 결과~ 이런식으로 해야함)
- 입력한 데이터가 너무 길면 허용치까지만 받아들여서 중복 되는 내용은 제거하고 각 그래프별 판단이 아니라 그냥 선택된 데이터간의 상관관계등 주요 인사이트만 뽑아서 출력
- 사용자가 평문 질문을 하면 기후, 질병, 환경에 대한 답변만 해주고, 그 외의 질문에는 "기후,환경,질병에 관한 질문만 가능합니다!"라고 답변
`;
