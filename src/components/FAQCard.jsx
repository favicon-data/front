...
//                   <th className="px-4 py-2 w-20" style={{ width: '6rem' }}>
//                     번호
//                   </th>
...
export default function FAQPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    fetch(`${API_BASE_URL}/faq/list`)
      .then((res) => {
        if (!res.ok) throw new Error('FAQ 데이터를 불러오지 못했습니다.');
        return res.json();
      })
      .then((data) => {
        // 서버 응답이 배열이면 그대로, 객체면 data.data 등으로 맞춰주세요
        const faqList = Array.isArray(data) ? data : data.data || [];
        setFaqs(faqList);
